...
const ProductDetail = () => {
  ...
  return (
    <div className="min-h-screen bg-brand-5 text-brand-1 py-12 px-4 relative overflow-hidden">
      {/* Decorative BG */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-4">
            <motion.div 
              ...
              className="aspect-square rounded-3xl overflow-hidden border border-brand-4 relative group"
            >
            ...
            <div className="grid grid-cols-3 gap-4">
              {product.images.map((img, idx) => (
                <button 
                  ...
                  className={`aspect-square rounded-2xl overflow-hidden border-2 transition-all ${activeImage === idx ? 'border-brand-3 opacity-100' : 'border-brand-4 opacity-50 hover:opacity-100'}`}
                >
          ...
          >
            <div className="inline-flex items-center gap-2 bg-brand-4/50 text-brand-2 px-4 py-2 rounded-full font-geist text-sm mb-6 border border-brand-3/30 w-fit">
              <ShieldCheck className="w-4 h-4" /> Aprobado ANMAT
            </div>
            
            <h1 className="text-5xl lg:text-7xl font-bebas text-brand-1 mb-4 leading-none drop-shadow-md">
              {product.name}
            </h1>
            
            <p className="font-bebas text-4xl text-brand-2 mb-8">${product.price}</p>
            
            <p className="font-geist text-lg text-brand-1/70 mb-8 leading-relaxed">
              {product.description}
            </p>
